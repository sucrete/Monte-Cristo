...
const Tab: React.FC<TabProps> = ({ index, children, className = '', variant = 'desktop' }) => {
  ...
  const getDesktopClassName = (): string => {
    return cn(
      'data-[state=selected]:text-secondary text-secondary/60 dark:data-[state=selected]:text-accent dark:text-accent/60 -mb-px cursor-pointer px-10 py-3 focus:outline-none focus-visible:ring-2 focus-visible:ring-bushwood-600/60 focus-visible:ring-offset-1 rounded-sm',
      className,
    );
  };
