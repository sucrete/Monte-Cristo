...
const PageMegaMenu = ({ className }: PageMegaMenuProps) => {
  return (
    <div>
      <div className="fixed top-full left-1/2 z-50 hidden w-full max-w-[1920px] -translate-x-1/2 overflow-hidden rounded-[20px] bg-transparent py-6 transition-all duration-200 group-focus-within/nav:flex group-hover/nav:flex dark:bg-transparent"></div>
      <div
        className={cn(
          'shadow-1 dark:border-background-5 border-stroke-4 dark:bg-background-6 pointer-events-none fixed top-full left-1/2 z-50 w-full max-w-[1920px] -translate-x-1/2 translate-y-10 overflow-hidden rounded-[20px] border bg-white opacity-0 transition-all duration-300 group-hover/nav:pointer-events-auto group-hover/nav:translate-y-0 group-hover/nav:opacity-100',
          className,
        )}>
